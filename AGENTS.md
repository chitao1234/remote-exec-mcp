...
and Winsock 2 path tested on Windows 95 and Windows 98 SE. In this repository,
"Windows XP-compatible" means a toolchain and binary target that can both
compile the C++ daemon as C++11 and target Windows XP.
...
- `remote-exec-daemon-cpp`: standalone C++11 daemon. It shares the
  broker-daemon protocol where implemented, supports optional OpenSSL or
  LibreSSL mTLS and POSIX BoringSSL mTLS, explicit plain HTTP, native POSIX and
  legacy Windows builds, and intentionally omits transfer compression.
