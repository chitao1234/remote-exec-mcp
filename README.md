...
- Broker and daemon forwarding limits protect open forwards, reconnecting forwards, retained sessions, retained listeners, active TCP streams, UDP peers/binds, and queued tunnel bytes. The Rust daemon enforces tunnel connection, retained session/listener, UDP bind, active TCP stream, and outbound queued-frame byte limits. The C++ daemon enforces its v4 retained session/listener, UDP bind, active TCP stream, outbound queued-frame byte, worker-thread, and tunnel I/O timeout limits.
