...
It supports optional OpenSSL or LibreSSL mutual TLS, POSIX BoringSSL mutual TLS,
and explicit plain HTTP:
...
| `remote-exec-daemon-cpp` | Standalone C++11 daemon with optional OpenSSL or LibreSSL mTLS, POSIX BoringSSL mTLS, explicit plain HTTP, and POSIX and legacy Windows build paths. |
...
- Plain HTTP targets must be configured with `allow_insecure_http = true` in the
  broker.
- The C++ daemon defaults to `TLS=auto`. POSIX builds compile and link TLS when
  OpenSSL 1.0.2 or newer, LibreSSL 2.7.1 or newer, or BoringSSL is available;
  the detected compatible library is used automatically. Windows XP and newer
  targets enable TLS automatically, while older Windows targets default to TLS
  off.
...
- Both daemons support reverse TLS and explicit insecure HTTP. The C++ daemon
  requires an OpenSSL- or LibreSSL-backed build, or a BoringSSL-backed POSIX
  build, for TLS.
...
| --- | --- |
| Transport | Optional OpenSSL or LibreSSL direct/reverse mTLS, BoringSSL direct/reverse mTLS on POSIX, or explicit plain HTTP. TLS builds use `https://...`; plain targets use `http://...` and `allow_insecure_http = true`. |
| Security | Mutual TLS with OpenSSL 1.0.2 or newer, LibreSSL 2.7.1 or newer, or BoringSSL on POSIX, optional bearer auth, certificate pinning, and static path sandboxing. |
...
with `OPENSSL_CPPFLAGS` and `OPENSSL_LDLIBS` pointing to the selected library's
headers and libraries. The `TLS=auto` probe recognizes LibreSSL by its
`LIBRESSL_VERSION_NUMBER` marker, so its OpenSSL-compatible version number is
not misclassified. When no supported library is available and TLS is not
needed, set `TLS=off` on the build and check commands.
