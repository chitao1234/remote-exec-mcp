...
- Broker and daemon forwarding limits protect open forwards, reconnecting forwards, retained sessions, retained listeners, active TCP streams, UDP peers/binds, and queued tunnel bytes. The broker uses the minimum advertised active TCP stream, UDP peer/bind, and queued-byte limits from the two daemon tunnels when reporting and enforcing a forward. The Rust daemon enforces tunnel connection, retained session/listener, UDP bind, active TCP stream, and outbound queued-frame byte limits. The C++ daemon enforces its v4 retained session/listener, UDP bind, active TCP stream, outbound queued-frame byte, worker-thread, tunnel I/O timeout, and TCP connect timeout limits; its active TCP stream limit counts established streams rather than pending connect attempts.
