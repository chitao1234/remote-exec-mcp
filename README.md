...
Run the C++ daemon when you need a smaller plain-HTTP daemon or legacy Windows
coverage:
...
| `remote-exec-daemon-cpp` | Standalone C++11 plain-HTTP daemon for POSIX and legacy Windows build paths. |
...
| `crates/remote-exec-daemon-cpp/config/daemon-cpp.example.ini` | C++ daemon listen address, default workdir, bearer auth, request/session/forwarding limits, yield-time limits, sandbox, and plain-HTTP reverse mode. |
...
- Plain HTTP targets must be configured with `allow_insecure_http = true` in the
  broker.
- C++ daemon targets are plain HTTP only and should usually use bearer auth when
  they are not isolated by another trusted transport.
...
- Rust reverse mode supports TLS and explicit insecure HTTP. The C++ daemon
  supports reverse mode over explicit plaintext HTTP only.
...
| --- | --- |
| Transport | Plain HTTP only. Broker targets must use `http://...` and `allow_insecure_http = true`. |
| Security | Optional bearer auth and static path sandboxing. No TLS. |
