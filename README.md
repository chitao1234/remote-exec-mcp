...
- `transfer_files` `transfer_mode` defaults to `lenient`, which skips unsupported source entries inside directory trees such as device nodes, FIFOs, sockets, and other special files, and reports structured/text warnings. Use `transfer_mode = "strict"` to fail instead.
...
- `remote-exec-daemon-cpp` is intentionally narrower than the main daemon: POSIX builds support `tty=true` when PTY allocation is available, Windows XP-compatible builds reject `tty=true`, `view_image` is unavailable, TLS and static sandboxing are unavailable, regular-file transfers, directory trees, and broker-built multi-source transfer bundles are supported, and transfer staging always falls back to uncompressed payloads. POSIX C++ builds can preserve, follow, skip, or reject source symlinks. Windows XP-compatible C++ builds do not support symlink preservation or following. On POSIX it follows the Rust daemon's default shell policy and forces `LC_ALL=C.UTF-8` plus `LANG=C.UTF-8`; on Windows XP-compatible builds it supports `cmd.exe`. Hard links, special files except for lenient skips during export, sparse entries, and malformed archive paths remain unsupported there.
