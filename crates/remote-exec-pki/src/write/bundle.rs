...
pub fn write_dev_init_bundle(
    ...
    let reverse_out_dir = out_dir.join("reverse");
    let reverse_daemon_out_dir = reverse_out_dir.join("daemons");
    fs::create_dir_all(out_dir).with_context(|| format!("creating {}", out_dir.display()))?;
    fs::create_dir_all(&daemon_out_dir)
        .with_context(|| format!("creating {}", daemon_out_dir.display()))?;
    fs::create_dir_all(&reverse_daemon_out_dir)
        .with_context(|| format!("creating {}", reverse_daemon_out_dir.display()))?;

    let ca = named_pair_paths(CA_PAIR_NAME, out_dir);
    let broker = named_pair_paths(BROKER_PAIR_NAME, out_dir);
    let reverse_broker = named_pair_paths(BROKER_PAIR_NAME, &reverse_out_dir);
    let manifest_path = out_dir.join("certs-manifest.json");
    let daemon_paths = build_daemon_paths(spec, &daemon_out_dir);
    let reverse_daemon_paths = build_daemon_paths(spec, &reverse_daemon_out_dir);
    validate_dev_init_output_paths(
        &ca,
        &broker,
        &reverse_broker,
        &daemon_paths,
        &reverse_daemon_paths,
        &manifest_path,
        force,
    )?;
    ...
    write_generated_pair(&reverse_broker, &bundle.reverse_broker, &mut written_paths)?;
    write_generated_daemon_pairs(spec, bundle, &daemon_paths, &mut written_paths)?;
    write_generated_reverse_daemon_pairs(spec, bundle, &reverse_daemon_paths, &mut written_paths)?;
...
fn validate_dev_init_output_paths(
    ...
    reverse_daemon_paths: &BTreeMap<String, KeyPairPaths>,
    manifest_path: &Path,
    force: bool,
) -> anyhow::Result<()> {
    validate_output_paths(
        std::iter::once(ca.cert_pem.as_path())
            ...
            .chain(std::iter::once(reverse_broker.cert_pem.as_path()))
            .chain(std::iter::once(reverse_broker.key_pem.as_path()))
            .chain(
                daemon_paths
                    .values()
...
fn write_generated_reverse_daemon_pairs(
    spec: &DevInitSpec,
    bundle: &GeneratedDevInitBundle,
    daemon_paths: &BTreeMap<String, KeyPairPaths>,
    ...
    for daemon in &spec.daemon_specs {
        ...
        let pem_pair = bundle
            .reverse_daemons
            .get(target)
            .with_context(|| format!("missing generated reverse daemon bundle for `{target}`"))?;
        let paths = daemon_paths
            .get(target)
            .with_context(|| format!("missing reverse daemon output paths for `{target}`"))?;
