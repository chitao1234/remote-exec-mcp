...
fn classify_listen_tunnel_event(result: anyhow::Result<Frame>) -> ForwardSideEvent {
    match result {
...
fn classify_terminal_tunnel_event(result: anyhow::Result<Frame>) -> ForwardSideEvent {
    match result {
        Ok(frame) if frame.frame_type == FrameType::Error => {
            ForwardSideEvent::TerminalTunnelError(decode_tunnel_error_frame(&frame))
        }
...
fn decode_tunnel_error_frame(frame: &Frame) -> TunnelErrorMeta {
...
fn format_terminal_tunnel_error(meta: &TunnelErrorMeta) -> anyhow::Error {
    let _ = meta.fatal;
