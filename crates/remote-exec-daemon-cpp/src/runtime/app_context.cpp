...
ServerRouteContext make_server_route_context(const DaemonConfig& config,
    ...
    const TargetInfoRouteContext target_info(
        config.target, metadata.daemon_instance_id, metadata.hostname, metadata.capabilities);
    const ExecRequestContext exec_request(
        paths, metadata.capabilities, metadata.default_shell, config.allow_login_shell);
    const ExecRouteContext exec(exec_request,
                                config.target,
                                services.sessions,
                                config.yield_time,
...
PortTunnelRouteContext make_port_tunnel_route_context(const DaemonConfig& config, AppServices& services) {
    return PortTunnelRouteContext(
        HttpGateContext(config.http_auth_bearer_token), config.port_forward_limits, services.port_tunnel);
}
...
HttpConnectionContext make_http_connection_context(const DaemonConfig& config,
    ...
    return HttpConnectionContext(make_server_route_context(config, metadata, sandbox, services),
