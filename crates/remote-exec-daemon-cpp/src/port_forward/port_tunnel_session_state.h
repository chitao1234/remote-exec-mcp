...
struct PortTunnelSessionAttachment {
    explicit PortTunnelSessionAttachment(
        const std::shared_ptr<PortTunnelConnection>& connection_value
    )
...
struct PortTunnelSession {
    PortTunnelSession(
        ...
        : session_id(session_id_value), service(service_value), state(PortTunnelSessionState::New),
          resume_deadline_ms(0ULL), generation(0ULL),
          retained_session_budget(std::move(retained_budget)), next_daemon_stream_id(2U) {}
    ...
    std::shared_ptr<PortTunnelSessionAttachment> detach_until(
    ...
    std::shared_ptr<PortTunnelConnection> connection_for_attachment(
    ...
    SessionRetainedInstallResult install_tcp_listener(
        uint32_t stream_id,
        const std::shared_ptr<RetainedTcpListener>& listener
    );
    SessionRetainedInstallResult install_udp_bind(
        uint32_t stream_id,
