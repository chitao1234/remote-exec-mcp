...
std::shared_ptr<PortTunnelSession> PortTunnelService::create_session() {
    if (!is_running()) {
        throw PortForwardError(
            400,
            "port_tunnel_shutting_down",
            "port tunnel service is shutting down"
        );
    }
...
void PortTunnelService::close_session(const std::shared_ptr<PortTunnelSession>& session) {
    ...
    log_message(
        LOG_DEBUG,
        "port_tunnel",
        LogMessageBuilder("session close requested")
            .quoted_field("session_id", session->session_id)
            .str()
    );
...
void PortTunnelService::close_all_sessions_for_shutdown() {
    ...
    {
        BasicLockGuard store_lock(mutex_);
        for (std::map<std::string, std::shared_ptr<PortTunnelSession>>::const_iterator it =
                 sessions_.begin();
             it != sessions_.end();
             ++it) {
...
std::shared_ptr<TunnelUdpSocket> PortTunnelService::session_udp_bind(
    const std::shared_ptr<PortTunnelSession>& session,
    uint32_t stream_id
) {
...
std::shared_ptr<PortTunnelSessionAttachment> PortTunnelService::wait_for_attachment(
    const std::shared_ptr<PortTunnelSession>& session
) {
