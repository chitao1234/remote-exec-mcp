...
environment, when a plain-HTTP daemon is acceptable, or when legacy Windows
...
| Transport | HTTP/1.1 JSON over plain HTTP only. No TLS. |
| Authentication | Optional HTTP bearer token. It authenticates requests but does not encrypt traffic. |
...
Broker targets that point at this daemon must use `http://...` plus
...
| XP-compatible | `nmake /f NMakefile all-msvc-xp` | `nmake /f NMakefile check-msvc-xp` | `test-msvc-xp-console-output`, `test-msvc-xp-session-store`, `test-msvc-xp-transfer`, `test-msvc-xp-server-routes-common`, `test-msvc-xp-server-runtime`, `test-msvc-xp-server-transport`, `test-msvc-xp-connection-manager` |
...
| GNU Windows XP/Winsock 2 | `build\remote-exec-daemon-cpp-xp-ws2.exe` |
| GNU Windows x64 XP/Winsock 2 | `build\remote-exec-daemon-cpp-x64-xp-ws2.exe` |
| GNU Windows 2000/Winsock 2 | `build\remote-exec-daemon-cpp-2000-ws2.exe` |
| GNU host-native Windows XP/Winsock 2 | `build\remote-exec-daemon-cpp-native-xp-ws2.exe` |
...
| MSVC XP-compatible | `build\msvc-xp\remote-exec-daemon-cpp-xp-msvc.exe` |
...
Reverse mode is exclusive and plain HTTP only for this daemon:
...
- Plain HTTP only. Optional bearer auth does not add confidentiality or
  integrity.
