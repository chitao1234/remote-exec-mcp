...
static void assert_descendant_cleanup_drain_stop_reason(bool descendant_cleanup_supported) {
    const SessionOutputDrainStopReason expected =
        descendant_cleanup_supported ? SessionOutputDrainStopReason::DescendantTerminateTimeout
                                     : SessionOutputDrainStopReason::DescendantTerminateUnsupported;

    LiveSession session;
    DrainMockProcessSession* process = new DrainMockProcessSession(descendant_cleanup_supported);
    ...
    const SessionOutputDrainResult result =
        drain_exited_session_output_locked(&session, &output, test_drain_policy(100UL, 0UL, 0UL));
    TEST_ASSERT(!result.completed);
    TEST_ASSERT(result.reason == expected);
    TEST_ASSERT(session.output_.last_drain_stop_reason == expected);
    TEST_ASSERT(session.output_.descendant_cleanup_attempted);
    TEST_ASSERT(session.output_.descendant_cleanup_supported == descendant_cleanup_supported);
    TEST_ASSERT(process->terminate_descendants_calls() == 1);

    const SessionOutputDrainResult second_result =
        drain_exited_session_output_locked(&session, &output, test_drain_policy(100UL, 0UL, 0UL));
    TEST_ASSERT(!second_result.completed);
    TEST_ASSERT(second_result.reason == expected);
    TEST_ASSERT(process->terminate_descendants_calls() == 1);
}

static void assert_explicit_drain_stop_reasons() {
    assert_descendant_cleanup_drain_stop_reason(false);
    assert_descendant_cleanup_drain_stop_reason(true);
...
static fs::path make_test_root() {
    return make_daemon_test_root("remote-exec-cpp-session-store-test");
}
...
static void assert_win32_process_tree_terminates_descendants(const fs::path& root) {
    ...
    const std::string child_command = "for /L %i in (1,1,200) do @echo tick>>"
                                      + test_exec_pty::windows_quote_arg(marker_path.string())
                                      + " & ping -n 2 127.0.0.1>nul";
    const std::string parent_command =
        "cmd.exe /D /C start \"remote-exec-tree-test\" /B cmd.exe /D /C "
        + test_exec_pty::windows_quote_arg(child_command) + " & ping -n 31 127.0.0.1>nul";
...
static Json poll_session(
    SessionStore& store,
    const std::string& daemon_session_id,
    const YieldTimeConfig& yield_time,
    const std::string& chars = std::string(),
    unsigned long yield_time_ms = 250UL
) {
    return write_test_stdin(
        store,
        daemon_session_id,
        chars,
        true,
...
static Json start_command_session(
    SessionStore& store,
    const fs::path& root,
    const std::string& command,
    const std::string& shell,
    ...
    unsigned long yield_time_ms,
    const YieldTimeConfig& yield_time,
    unsigned long max_open_sessions = 64UL
) {
    return start_test_command(
        store,
        command,
        root.string(),
        shell,
...
static std::string append_running_session_output_until_pty_size(
    ...
    while (!test_exec_pty::pty_size_output_matches(output, rows, cols)
        ...
        const Json poll = poll_session(store, daemon_session_id, yield_time, "", 250UL);
...
static Json poll_session_until_done(
    ...
    while (response.at("running").get<bool>() && platform::monotonic_ms() - started < timeout_ms) {
        platform::sleep_ms(10UL);
        response = poll_session(store, daemon_session_id, yield_time, "", 250UL);
        if (output != nullptr) {
...
static std::string append_session_output_until_done(
    ...
    const std::uint64_t started = platform::monotonic_ms();
    do {
        poll = poll_session(store, daemon_session_id, yield_time, "", 250UL);
        output += normalize_output(poll.at("output").get<std::string>());
        if (!poll.at("running").get<bool>()) {
...
static std::string run_windows_quote_sensitive_command(
    ...
    const Json started =
        start_command_session(store, root, command, shell, tty, 5000UL, yield_time);
...
static void assert_windows_cmd_runs_quoted_script_path_with_spaces(
    ...
    const Json response = start_command_session(
        store,
        root,
...
#ifndef _WIN32
static Json start_sessions_until_warning(
    SessionStore& store,
    const fs::path& root,
    const std::string& shell,
    unsigned long max_open_sessions
...
static void assert_unknown_session(
    ...
    bool rejected = false;
    try {
        (void)poll_session(store, daemon_session_id, yield_time, "", 1UL);
...
static void assert_completed_command_output(
    ...
    const Json response =
        start_command_session(store, root, merge_command, shell, false, 5000UL, yield_time);
...
static void assert_token_limiting(
    ...
    const Json omitted_limit =
        start_command_session(store, root, print_huge_command, shell, false, 5000UL, yield_time);
...
static void assert_posix_exit_drain_boundaries(
    ...
    const Json noisy_descendant = start_command_session(
        store,
        root,
    ...
    const Json stdout_held_open = start_command_session(
        store,
        root,
...
static bool wait_until_session_exits(
    ...
    const std::uint64_t started = platform::monotonic_ms();
    while (platform::monotonic_ms() - started < timeout_ms) {
        const Json poll = poll_session(store, session_id, yield_time, "", 1UL);
        if (!poll.at("running").get<bool>()) {
            return true;
        }
        platform::sleep_ms(10UL);
...
static void assert_posix_sigchld_reaper_reaps_exited_session_children(
    ...
    for (int index = 0; index < 5; ++index) {
        const Json running = start_command_session(
            zombie_store,
            root,
...
static void assert_posix_sigchld_reaper_preserves_exit_status_during_pty_resume_race(
    ...
    for (int attempt = 0; attempt < 10; ++attempt) {
        ...
        const Json resumed = poll_session(race_store, waiting_id, fast_yield, "ping\n", 1000UL);
...
static void assert_non_tty_stdin_closed_rejected(
    ...
    const Json non_tty_running = start_command_session(
        store,
        root,
...
#endif
    const Json waiting =
        start_command_session(store, root, command, shell, true, 1000UL, fast_yield);
    TEST_ASSERT(waiting.at("running").get<bool>());
    const std::string session_id = waiting.at("daemon_session_id").get<std::string>();

    Json completed = poll_session(
        store,
        session_id,
        fast_yield,
        test_exec_pty::terminal_input_line("done"),
        5000UL
    );
...
static void assert_tty_resume_round_trip(
    ...
    Json resumed = poll_session(
        store,
        waiting_id,
        fast_yield,
        test_exec_pty::terminal_input_line("ping"),
        1000UL
    );
    ...
    while (resumed.at("running").get<bool>() && platform::monotonic_ms() - resume_started < 5000UL
        ...
        resumed = poll_session(store, waiting_id, fast_yield, "", 250UL);
...
static void assert_unrelated_sessions_do_not_block_each_other(
...
#endif
    const Json slow_running =
        start_command_session(store, root, slow_command, shell, true, 250UL, yield_time);
    ...
    const Json fast_running =
        start_command_session(store, root, fast_command, shell, true, 250UL, yield_time);
    ...
    TEST_ASSERT(test_assert::wait_until_true(slow_thread_started, 1000UL));
    const std::uint64_t fast_started_at = platform::monotonic_ms();
    const Json fast_completed = poll_session(
        store,
...
static void assert_tty_resize_round_trip(
    ...
    const Json resized = write_test_stdin(
        ...
        test_exec_pty::terminal_input_line(""),
        true,
        1000UL,
        DEFAULT_MAX_OUTPUT_TOKENS,
        fast_yield,
...
#endif
    const Json non_tty_running =
        start_command_session(store, root, command, shell, false, 250UL, yield_time);
    TEST_ASSERT(non_tty_running.at("running").get<bool>());
...
static void assert_recent_session_survives_limit_prune(
    ...
    const Json first_touch = poll_session(
        recency_store,
        first_running.at("daemon_session_id").get<std::string>(),
        fast_yield,
        "",
...
static void assert_recent_session_is_protected_from_prune(
    ...
    TEST_ASSERT(poll_session(protected_store, daemon_session_ids[0], fast_yield, "", 1UL)
                    .at("running")
                    .get<bool>());
...
static void assert_stdin_and_tty_behavior(
    ...
    const Json cmd_running =
        start_command_session(store, root, "cmd.exe", shell, true, 1000UL, yield_time);
    ...
    Json cmd_response = poll_session(store, cmd_session_id, yield_time, "echo hello\r\n", 5000UL);
    cmd_output += normalize_output(cmd_response.at("output").get<std::string>());
    cmd_response = poll_session(store, cmd_session_id, yield_time, "exit\r\n", 5000UL);
    cmd_output += normalize_output(cmd_response.at("output").get<std::string>());
...
static void assert_threshold_warnings_and_unknown_sessions(
...
#else
    {
        ...
        start_sessions_until_warning(warning_store, root, shell, DEFAULT_MAX_OPEN_SESSIONS);
    }

    bool unknown_session_rejected = false;
    try {
        (void)poll_session(store, "missing-session", yield_time, "", 250UL);
    } catch (const UnknownSessionError&) {
...
#else
    SessionStore warning_store;
    start_sessions_until_warning(warning_store, root, shell, 6UL);
#endif
}
