...
static void assert_exec_routes(TestRouteHarness& harness, const fs::path& root) {
    ...
    if (!test_exec_pty::should_skip_pty_tests(process_session_supports_pty())) {
        ...
        Json completed =
            exec_write_json(harness, started_session_id, test_exec_pty::terminal_input_line("hello"), 5000UL);
        ...
        if (!test_exec_pty::pty_size_output_matches(resize_output, 33U, 101U)) {
            resize_output =
                append_running_exec_output_until_pty_size(harness, resize_session_id, resize_output, 33U, 101U, 2000UL);
        }
