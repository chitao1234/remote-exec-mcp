...
void test_wide_to_utf8() {
    ...
    TEST_ASSERT(
        utf8_from_windows_wide_for_test(wide_from_units(unpaired_high, 3U)) == "x\xEF\xBF\xBD"
                                                                               "y"
    );

    const unsigned int unpaired_low[] = {'x', 0xDE00U, 'y'};
    TEST_ASSERT(
        utf8_from_windows_wide_for_test(wide_from_units(unpaired_low, 3U)) == "x\xEF\xBF\xBD"
                                                                              "y"
    );
}
...
void test_terminal_output_filter_strips_winpty_control_sequences() {
    ...
    TEST_ASSERT(
        filter_terminal_output_for_test(&filter, "\x1b[0m\x1b[0Khello\x1b[0K\x1b[?25l\r\n\x1b[0K\x1b[?25h") == "hello\n"
    );
...
void test_terminal_output_filter_strips_osc_title_sequences() {
    TerminalOutputFilter filter;
    TEST_ASSERT(
        filter_terminal_output_for_test(&filter, "\x1b]0;C:\\Windows\\system32\\cmd.exe\x07hello \r\n") == "hello\n"
    );
    TEST_ASSERT(drain_terminal_output_for_test(&filter).empty());
}
...
void test_terminal_output_filter_collapses_winpty_prompt_rewrites() {
    ...
    TEST_ASSERT(
        final.find("\xE7\x89\x88\xE6\x9D\x83\xE6\x89\x80\xE6\x9C\x89 1985-2001 Microsoft Corp.") != std::string::npos
    );
...
void test_terminal_output_filter_does_not_finalize_partial_rows_on_flush_due() {
    ...
    TEST_ASSERT(filter_terminal_output_at_for_test(&filter, "prompt>echo hello\x1b[116Ghell\r\no", 1000U).empty());
    TEST_ASSERT(
        normalize_winpty_transcript_chunk_for_test(&normalizer, flush_terminal_output_due_for_test(&filter, 1100U))
            .empty()
    );
    TEST_ASSERT(normalize_winpty_transcript_chunk_for_test(
                    &normalizer,
                    filter_terminal_output_at_for_test(&filter, "\r\x1b[1Aprompt>echo hello\x1b[0K\r\nhello\r\n", 1150U)
    )
                    .empty());
    TEST_ASSERT(
        normalize_winpty_transcript_chunk_for_test(&normalizer, flush_terminal_output_due_for_test(&filter, 1299U))
            .empty()
    );
...
void test_winpty_transcript_normalizer_reconstructs_banner_and_prompt() {
    ...
    const std::string physical =
        ...
        "    C:\\chi\n"
        "\\winpty-probe>\n";
    const std::string expected = "Microsoft Windows XP [\xE7\x89\x88\xE6\x9C\xAC 5.1.2600]\n"
...
void test_winpty_transcript_normalizer_splits_echo_output_from_prompt_repaint() {
    WinptyTranscriptNormalizer normalizer;
    const std::string physical = "\\winpty-probe>echo hello                                                            "
                                 "                              hello\n"
                                 "                                                                                     "
                                 "                             C:\\chi\n"
                                 "\\winpty-probe>\n";
...
void test_winpty_transcript_normalizer_updates_configured_width() {
    ...
    const std::string expected = stale_width_physical + "C:\\p>echo hello\n"
                                                        "hello\n"
                                                        "C:\\p>\n";
    TEST_ASSERT(actual == expected);
}
